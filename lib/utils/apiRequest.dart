...
Future<dynamic> getSkills() async {
  ...
  try {
    ...
    print("After defining the client object");
    final response =
        await client.get(Uri.https(url, "/api/v1/skills"), headers: headers);
...
Future<dynamic> getOffers() async {
  ...
  try {
    ...
    print("After defining the client object");
    final response =
        await client.get(Uri.https(url, "/api/v1/offers"), headers: headers);
...
Future<dynamic> myProjects() async {
  ...
  try {
    ...
    print("After defining the client object");
    final response =
        await client.get(Uri.https(url, "/api/v1/projects"), headers: headers);
    // mywidgets.displayToast(msg: "after making the get request");
    print("Before decoding the request");
    final utf8Response = utf8.decode(response.bodyBytes);
    final jsonData = json.decode(utf8Response) as Map;
...
Future<dynamic> createPost({
  ...
  try {
    List files = [];
    for (var value in imageFiles!) {
      String fileName = File(value!.path).path.split('/').last;
      files.add(await MultipartFile.fromFile(File(value.path).path,
          filename: fileName));
    }
    ...
    final responseString = await dio.post("${mainUrl}/status",
        data: formData,
        options: Options(headers: {'Authorization': 'Bearer $token'}));
        print(responseString.data);
    // final jsonData = json.decode(responseString.data);
